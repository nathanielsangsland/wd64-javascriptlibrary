...
* **************
    ...
    - scope is how a compter keeps track of all the varianles in a program.
    - scope can also be nested, where there's an outer scope (global) that encloses an inner scope.
*/

let scope = "Earth"; // this currently in the global scope of our JS file.

function localScope() {
    let scope = "Indiana"; // the state (local) within the JS "World/Globe"
   // console.log(`${scope} mark the local scope`);
...
//! ORDER MATTERS
// JS reads from top to bottom and acts according to waht is being asked.  JS is known as a single-threaded language which means it can only execute a single task at a time.
...
// statments like "if" or loops don't create a new scope. If a variable is defined inside a block statment, it will remain in that scope.  However, we can utilize let an const as keywords to help us control how our varibles are viewed.
